...
def generate_optimized_density_match_L2_gradient(image):
    ...
    def density_match_L2_gradient_2d(sigma, sqrtJ, dsqrtJdx, dsqrtJdy,
                                     W, dWdx, dWdy, W0, dW0dx, dW0dy,
                                     doutdx, doutdy):
        for i in range(s):
            for j in range(s):
...
class CompatibleConformalDensityMatching(object):
    """
    Implementation of the comptible matching algorithm using
    the conformal metric.

    Objects from this class are efficient implementations of
    the diffeomorphic density matching algorithm detailed in
    the paper by Bauer, Joshi, and Modin (2015).
    ...
    def __init__(self, source, target, compute_phi=True):
        ...
        alpha : float
            Parameter specifying the midpoint to be used for
            the conformal metric.
        compute_phi : bool
    ...
    def run(self, epsilon=0.1, alpha=0.5, use_conformal=True):
        ...
        for t in tvec:
            ...
            self.diffeo_compose(self.phiinvx, self.phiinvy,
                                self.psiinvx, self.psiinvy,
                                self.tmpx, self.tmpy)
...
class TwoComponentDensityMatching(object):
    ...
    Objects from this class are efficient implementations
    of the diffeomorphic density matching algorithm detailed
    in the paper by Bauer, Joshi, and Modin (2015).
    The computations are accelerated using the `numba` library.
    """
    ...
    def __init__(self, source, target, sigma=0.5, compute_phi=True):
        ...
        # Check input
        if (I0.shape != I1.shape):
            raise(TypeError('Source and target must have the same shape.'))
        if (I0.dtype != I1.dtype):
            raise(TypeError('Source and target must have the same dtype.'))
        if (sigma < 0):
            raise(ValueError('Paramter sigma must be positive.'))
        for d in I1.shape:
            if (d != I1.shape[0]):
                raise(NotImplementedError('Only square image allowed so far.'))
        if (len(I1.shape) != 2):
            raise(NotImplementedError('Only 2d images allowed so far.'))
        ...
        self.image_gradient = generate_optimized_image_gradient(I1)
        self.divergence = generate_optimized_divergence(I1)
        self.density_match_L2_gradient = \
            generate_optimized_density_match_L2_gradient(I1)
        self.evaluate = generate_optimized_diffeo_evaluation(I1)
        ...
        self.dtmpdx = np.zeros_like(I1)
        self.dtmpdy = np.zeros_like(I1)
        ...
        self.discr_space = odl.uniform_discr([-16, -16],
                                             [16, 16], [64, 64],
                                             dtype='float32', interp='linear')

        # Create the ground truth as the given image
        self.ground_truth = self.discr_space.element(I0.T)

        # Create the template as the given image
        self.template = self.discr_space.element(I1.T)

        # Compose mass-preserving operator to template
        self.template_mass_pre = self.template
        ...
        self.angle_partition = odl.uniform_partition(
            0, np.pi, self.num_angles, nodes_on_bdry=[(True, False)])

        # Create 2-D projection domain
        self.detector_partition = odl.uniform_partition(-24, 24, 192)
        ...
        self.xray_trafo_op = odl.tomo.RayTransform(self.discr_space,
                                                   self.geometry,
                                                   impl='astra_cuda')

        # Create projection data by given setting
        self.proj_data = self.xray_trafo_op(self.ground_truth)

        # Create white Gaussian noise
        self.noise = 5.0 * self.proj_data.space.element(
            proj_noise(self.proj_data.shape))

        # Output the signal-to-noise ratio
        print('snr = {!r}'.format(SNR(self.proj_data, self.noise)))

        # Create noisy projection data
        self.noise_proj_data = self.proj_data + self.noise

        # Do the backprojection reconstruction
        self.backproj = self.xray_trafo_op.adjoint(self.noise_proj_data)

        self.density_match_L2_gradient_rec = \
    ...
    def run(self, niter=300, epsilon=0.1, yielditer=False):
        ...
        for k in range(niter):
            ...
            self.density_match_L2_gradient(self.sigma, self.sqrtJ,
                                           self.dsqrtJdx, self.dsqrtJdy,
                                           self.W, self.dWdx, self.dWdy,
                                           self.W0, self.dW0dx, self.dW0dy,
                                           self.vx, self.vy)
...
#          print('Mean v after: [%f,%f]'%(-np.mean(self.vx),-np.mean(self.vy)))
            ...
            # Compute forward phi also (only for output purposes)
            if self.compute_phi:
                self.diffeo_compose(self.phix, self.phiy, self.psix,
                                    self.psiy, self.tmpx, self.tmpy)
                np.copyto(self.phix, self.tmpx)
                np.copyto(self.phiy, self.tmpy)
    ...
    def run_rec(self, niter=300, epsilon=0.1, yielditer=False):
        """
        Carry out the matching process.
        ...
        N/A
        """
        self.W0 = self.I0.T.copy()
        self.W1 = self.I1.T.copy()

        self.W1 *= np.linalg.norm(self.W0, 'fro')/np.linalg.norm(self.W1, 'fro')

        self.W = self.W1
        self.template = self.discr_space.element(self.W)
        ...
        for k in range(niter):
            ...
            # OUTPUT
            self.E[k+kE] = (self.sigma*(self.sqrtJ - 1)**2).sum()

            # STEP 1: update template_mass_pre
            self.template_array = np.asarray(self.template, dtype='float64')
            template_mass_pre_array = np.asarray(self.template_mass_pre,
                                                 dtype='float64')
            self.image_compose(self.template_array, self.phiinvx,
                               self.phiinvy, template_mass_pre_array)

            template_mass_pre_array *= self.J
            self.W = template_mass_pre_array
            self.template_mass_pre = self.discr_space.element(self.W)

            # STEP 2: compute the L2 gradient
            tmpx = self.xray_trafo_op(self.template_mass_pre)
            tmpx -= self.noise_proj_data

            self.E[k+kE] += np.asarray(tmpx**2).sum()

            tmpx = self.xray_trafo_op.adjoint(tmpx)
            tmpx_array = np.array(tmpx, dtype='float64')
            self.image_gradient(tmpx_array, self.dtmpdx, self.dtmpdy)
            self.dtmpdx *= self.W
            self.dtmpdy *= self.W
            ...
            self.density_match_L2_gradient_rec(self.sigma,
                                               self.dsqrtJdx, self.dsqrtJdy,
                                               self.dtmpdx, self.dtmpdy,
                                               self.vx, self.vy)
            ...
            # Compute forward phi also (only for output purposes)
            if self.compute_phi:
                self.diffeo_compose(self.phix, self.phiy,
                                    self.psix, self.psiy,
                                    self.tmpx, self.tmpy)
                np.copyto(self.phix, self.tmpx)
                np.copyto(self.phiy, self.tmpy)
